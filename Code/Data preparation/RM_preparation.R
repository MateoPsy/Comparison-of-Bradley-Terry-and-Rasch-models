...
### creating combined binarized dataset for FA ####
all_combined <- read_csv("Data/2PL/all_combined.csv")

head(all_combined)

all_combined_bi <- all_combined %>%
  mutate(across(V1:V8, ~ case_when(
    # collection 1
    collection == 1 & . %in% c(1,2) ~ -1,
    collection == 1 & . %in% c(3,4) ~ 1,
    
    # collection 2 – dataset-specific keys (uppercase dataset_id)
    collection == 2 & dataset_id == "AUT" & .x == 1 ~ -1,
    collection == 2 & dataset_id == "AUT" & .x == 2 ~ 1,
    
    collection == 2 & dataset_id == "BMPN" & .x %in% c(1,2) ~ -1,
    collection == 2 & dataset_id == "BMPN" & .x == 3 ~ NA_real_,
    collection == 2 & dataset_id == "BMPN" & .x %in% c(4,5) ~ 1,
    
    collection == 2 & dataset_id == "DASS" & .x %in% c(0,1) ~ -1,
    collection == 2 & dataset_id == "DASS" & .x %in% c(2,3) ~ 1,
    
    collection == 2 & dataset_id == "DEM" & .x %in% c(1,2,3) ~ -1,
    collection == 2 & dataset_id == "DEM" & .x %in% c(5,6,7) ~ 1,
    
    collection == 2 & dataset_id == "DERS" & .x %in% c(1,2) ~ -1,
    collection == 2 & dataset_id == "DERS" & .x == 3 ~ NA_real_,
    collection == 2 & dataset_id == "DERS" & .x %in% c(4,5) ~ 1,
    
    collection == 2 & dataset_id == "HI" & .x %in% c(1,2) ~ -1,
    collection == 2 & dataset_id == "HI" & .x %in% c(3,4) ~ 1,
    
    TRUE ~ as.numeric(.)  # keep all other values unchanged
  )))

head(all_combined_bi)
head(all_combined)
write.csv(all_combined_bi, "Data/2PL/all_combined_bi.csv", row.names = FALSE)
